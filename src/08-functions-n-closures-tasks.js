...
function getPolynom(...args) {
  return (x) => {
    ...
    return arr.reduce((prev, curr, index) => prev + curr * (x ** (index + 1)), lastItem);
  };
}
...
function retry(func, attempts) {
  ...
  function retFunc() {
    ...
    try {
      ...
      counter += 1;
      if (counter < attempts) result = retFunc();
    }
    return result;
  }
  return retFunc;
...
function getIdGeneratorFunction(startFrom) {
  ...
  return () => {
    ...
    if (map.has(startFrom)) {
      res = map.get(startFrom) + 1;
      map.set(startFrom, res);
    } else { map.set(startFrom, startFrom); }
